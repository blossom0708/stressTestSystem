...
<br> 
**友情提示：** 本平台支持script脚本压测和jmeter api压测两种模式，大家可以在配置中[系统管理->参数配置]中修改。
 - script脚本压测模式：不支持ECharts实时监控，但支持配合grafana + influxdb实时监控，相当于是jmeter的non-gui模式；
 - jmeter api压测模式：支持ECharts实时监控，但不建议多脚本同时运行并监控（原来的功能就是一个脚本调用所有节点，并收集所有节点的监控数据进行合并），同样也不支持多脚本同时运行的grafana和influxdb监控（道理同ECharts实时监控）；
 - 需要说明的是如果有人想个性化（比如不同节点运行不同脚本），建议用script脚本压测模式，结合grafana + influxdb监控，就能同时监控多个脚本的压测数据，这些数据不会被合并处理。
<br> 